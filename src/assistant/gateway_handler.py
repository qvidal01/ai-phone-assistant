...
class GatewayHandler:
    ...
    def generate_response(
        ...
        try:
            ...
            self.logger.info(f"Generated response using Gateway/{model} for: {user_message[:50]}...")
            ...
            with httpx.Client(timeout=self.timeout) as client:
                response = client.post(
                    ...
                    json={
                        ...
                        "messages": [{"role": "system", "content": "Summarize conversations briefly."}] + summary_messages,
                        "max_tokens": 256,
                    }
                )
                response.raise_for_status()
                ...
                result = response.json()
                return result.get("choices", [{}])[0].get("message", {}).get("content", "Unable to generate summary.")
