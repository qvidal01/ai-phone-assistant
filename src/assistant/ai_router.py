...
class AIRouter:
    ...
    def __init__(
        ...
        self.appointment_keywords = [
            ...
            "cancel", "reschedule", "change", "available",
            "slot", "time", "date", "calendar"
        ]
    ...
    def route(self, query: str, context: Optional[Dict] = None) -> RoutingDecision:
        ...
        if gateway_up and not ollama_up and not claude_up:
            ...
            return RoutingDecision(
                backend=BackendType.GATEWAY_FAST if complexity == QueryComplexity.SIMPLE else BackendType.GATEWAY_SMART,
                model=model,
        ...
        if ollama_up and not claude_up and not gateway_up:
            ...
            return RoutingDecision(
                backend=BackendType.OLLAMA_FAST if complexity == QueryComplexity.SIMPLE else BackendType.OLLAMA_CHAT,
                model=model,
        ...
        # VOICE OPTIMIZATION: Use fast models for ALL voice queries to minimize latency
        if complexity == QueryComplexity.SIMPLE:
            # Simple queries → fast local model (< 2 seconds)
            return RoutingDecision(
            ...
            # For voice calls, ALWAYS use fast model to minimize latency
            # The quick-responder can handle most moderate queries adequately
            if self.prefer_local:
                return RoutingDecision(
                    backend=BackendType.OLLAMA_FAST,
                    model="quick-responder:latest",
                    reason="Voice-optimized: using fast model for low latency",
                    complexity=complexity
                )
            else:
                # If Claude is available and preferred, use it for quality
                return RoutingDecision(
                    backend=BackendType.CLAUDE,
                    model=None,
            ...
            # Complex queries - try Claude first (faster API), fallback to local
            if claude_up:
                return RoutingDecision(
                    backend=BackendType.CLAUDE,
                    model=None,
                    reason="Complex query, using Claude for best quality and speed",
                    complexity=complexity
                )
            elif gateway_up:
                return RoutingDecision(
                    backend=BackendType.GATEWAY_SMART,
                    model="llama-3.1-8b",
                    reason="Complex query, using AI Gateway (Claude unavailable)",
                    complexity=complexity
                )
            else:
                # Fallback to fast local model even for complex - latency matters
                return RoutingDecision(
                    backend=BackendType.OLLAMA_FAST,
                    model="quick-responder:latest",
                    reason="Complex query but using fast model for voice latency",
                    complexity=complexity
                )
