...
class Config(BaseModel):
    ...
    ollama_url: str = Field(
        "http://192.168.0.234:11434",
        description="Ollama API base URL"
    )
    ollama_default_model: str = Field(
        "quick-responder:latest",
        description="Default Ollama model for fast responses"
    )
    ollama_smart_model: str = Field(
        "llama3.3:70b",
        description="Ollama model for complex queries"
    )
    ollama_chat_model: str = Field(
        "cyberque-chat:latest",
        description="Ollama model for general chat"
    )

    # AI Gateway Configuration (Cloudflare Workers AI)
    ai_gateway_url: str = Field(
        "https://ai-gateway.aiqso.io",
        description="AIQSO AI Gateway URL"
    )
    ai_gateway_default_model: str = Field(
        "auto",
        description="AI Gateway default model (auto for smart routing)"
    )
    ai_gateway_enabled: bool = Field(
        True,
        description="Enable AI Gateway as a backend option"
    )

    # AI Routing Configuration
    prefer_local_ai: bool = Field(
        True,
        description="Prefer local Ollama over Claude when quality is similar"
    )
    prefer_edge_ai: bool = Field(
        False,
        description="Prefer edge AI Gateway over local Ollama for simple queries"
    )
    ...
    google_calendar_credentials: Optional[str] = Field(
        None,
        description="Path to Google Calendar credentials JSON"
    )
    calendar_id: Optional[str] = Field(
        None,
        description="Google Calendar ID for appointments"
    )
    ...
    odoo_database: Optional[str] = Field(
        None,
        description="Odoo database name"
    )
    odoo_username: Optional[str] = Field(
        None,
        description="Odoo username"
    )
    odoo_password: Optional[str] = Field(
        None,
        description="Odoo password/API key"
    )

    # EasyAppointments Integration
    easyappointments_url: Optional[str] = Field(
        None,
        description="EasyAppointments URL (cal.aiqso.io)"
    )
    easyappointments_token: Optional[str] = Field(
        None,
        description="EasyAppointments API token"
    )

    # Escalation settings
    escalation_phone: Optional[str] = Field(
        None,
        description="Phone number for escalating to human"
    )

    # Voice Settings
    tts_voice: str = Field(
        "female_us",
        description="TTS voice: male_us, female_us, male_uk, female_uk, female_aus"
    )
    ...
    business_type: Optional[str] = Field(
        None,
        description="Business type (auto_shop, salon, medical, general)"
    )
...
def load_config() -> Config:
    ...
    config_dict = {
        ...
        # EasyAppointments
        "easyappointments_url": os.getenv("EASYAPPOINTMENTS_URL", "https://cal.aiqso.io"),
        "easyappointments_token": os.getenv("EASYAPPOINTMENTS_TOKEN"),

        # Escalation
        "escalation_phone": os.getenv("ESCALATION_PHONE"),

        # Voice
        "tts_voice": os.getenv("TTS_VOICE", "female_us"),
